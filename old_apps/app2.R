...
ui <- shinyUI(fluidPage(theme = shinytheme("cyborg"),
                        fluidRow(
                          column(12,titlePanel(tags$div(h3(style="text-align:center;color:white","Using temporally explicit habitat suitability models to assess threats to mobile species and evaluate the effectiveness of marine protected areas ")))),
                          column(12,titlePanel(tags$div(h4(style="text-align:center;",HTML(paste("Heather Welch",tags$sup(1,"*"),", ","Robert L. Pressey, ",tags$sup(2,3,"*","°"),", ","April E. Reside",tags$sup(4)))))),
                          ...
                          fluidRow(
                            ...
                            column(3,leafletOutput("all"))
                                   ...
                                   dateRangeInput("date","Select display date",min = "1998-01",max = "2007-12",format="yyyy-mm",separator = "-")),

                          
                        tabPanel("test2",titlePanel("Explore MaxEnt models by climatological month"))
)))))
                                 #           fluidRow(
                                 #             column(12,
                                 #                    style = "text-align:center;border-radius: 3px; padding: 0px; border: 1px solid; background: white; ",
                                 #                    titlePanel(tags$div(h3(style="text-align:center;color:black","TEST"))))),
                                 #           column(12,
...
server <- shinyServer(function(input, output) {
  ...
  output$bs <- renderLeaflet({
    ...
    lmap <-setView(lmap,ras_ext$lon,ras_ext$lat,zoom=3)
